...
class KMeans:
    ...
    def _init_centroids(self):
        ...
        if self.options['km_init'].lower() == 'first':
            ...
            rang = len(indexes) - self.K
            self.centroids = self.old_centroids = self.X[indexes[rang:]]
            
        elif self.options['km_init'].lower() == 'last':
            indexes = np.sort(np.unique(self.X, return_index=True, axis=0)[1])
            self.centroids = self.X[indexes[-self.K:]]
            self.old_centroids = self.centroids.copy()
            

        elif self.options['km_init'].lower() == 'mid':
            indexes = np.sort(np.unique(self.X, return_index=True, axis=0)[1])
            lenInd = len(indexes)
    ...
    def get_centroids(self):
        ...
        for j in range(self.K):
            suma = np.sum(self.X[np.where(self.labels == j)],axis = 0, dtype = np.float64)
            n = self.X[np.where(self.labels == j)]
    ...
    def find_bestK(self, max_K):
        ...
        for self.K in range(3, max_K):
            ...
            if (self.options['limit'] / 100) > quocient:
                self.K -= 1
                break
...
def retrieval_by_color(imatges, etiquetes, pregunta):
    ...
    for imatge in imatges:
        color_imatge = get_colors(imatge)
        for color in color_imatge:
            if (color == pregunta):
                np.append(llista, imatge)
                break
    return llista
'''
def retrieval_by_color(imatges, etiquetes, pregunta):
